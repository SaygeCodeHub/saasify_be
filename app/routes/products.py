...
def add_product_variant(product_data: schemas.ProductVariant, product_id: int = Query(None, description="id for the product"), db: Session = Depends(get_db)):
    try:
        ...
        new_variant = models.ProductVariant(
            ...
            product_id=existing_product_variant.product_id,
            barcode_no=product_data.barcode_no,
            image=product_data.image,
            is_published=product_data.is_published
        )
        new_stock = models.Stock(
            product_id=existing_product_variant.product_id,
            variant_id=existing_product_variant.variant_id,
            current_stock_count=new_variant.stock,
            barcode_no=product_data.barcode_no
        )
        db.add(new_stock)
...
def edit_product_variant(variant_id: int, variant_data: schemas.ProductEdit, db: Session = Depends(get_db)):
    try:
        existing_variant = db.query(models.ProductVariant).filter(
            models.ProductVariant.variant_id == variant_id, models.ProductVariant.branch_id == variant_data.branch_id).first()
...
def edit_product(product_data: schemas.ProductUpdate, product_id: int = Query(...), db: Session = Depends(get_db)):
    try:
        ...
        product = db.query(models.Products).filter(models.Products.product_name == product_data.product_name).first()
        if product:

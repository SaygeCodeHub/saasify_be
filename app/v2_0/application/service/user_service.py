...
def add_user(user, db):
    ...
    user_email_exists = db.query(models_2.Users).filter(
        models_2.Users.user_email == user.user_email).first()
    user_contact_exists = db.query(models_2.Users).filter(
        models_2.Users.user_contact == user.user_contact).first()
    ...
    new_user = models_2.Users(**user.model_dump())
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def set_modified_by(new_user, db):
    ...
    db.query(models_2.Users).filter(models_2.Users.user_id == new_user.user_id).update(
...
def modify_user(user, user_id, db):
    """Updates a User"""
    user_query = db.query(models_2.Users).filter(models_2.Users.user_id == user_id)
